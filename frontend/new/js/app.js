...
const AppState = {
    ...
    isCheckingNotifications: false, // 防止并发通知检查
    lastDataRefresh: 0, // 最后数据刷新时间
    dataRefreshCooldown: 5000 // 数据刷新冷却时间（5秒）
};
                ...
                for (const notification of giftNotifications) {
                    ...
                    try {
                        ...
                        if (checkResult.success && checkResult.should_display) {
                            console.log('后端允许显示弹窗:', checkResult);
                            ...
                            console.log('后端不允许显示弹窗:', checkResult);
                        }
                    } catch (error) {
                        console.error('处理赠送通知失败:', error);
                    }
                }
        ...
        if (data.success && data.records) {
            ...
            data.records.forEach(record => {
                ...
                if (!seenRecords.has(recordKey)) {
                    ...
                    console.warn('发现重复记录:', recordKey);
                }
            });

            AppState.recentCodes = processedCodes;
            console.log('处理后的兑换码数据:', AppState.recentCodes);
...
function emergencyStopNotifications() {
    try {
        ...
        modals.forEach(modal => {
            modal.classList.remove('active');
            console.log('已关闭弹窗:', modal.id);
        });

        console.log('所有通知检查已紧急停止');
        alert('🛑 所有通知检查已紧急停止！\n页面将不再自动检查通知，需要手动刷新页面恢复正常功能。');
    } catch (error) {
...
async function testModalControl() {
    try {
        ...
        if (checkResult.success && checkResult.should_display) {
            ...
            // 3. 显示测试弹窗
            showGiftModal(testCode, 10, '这是弹窗控制系统测试');

            alert('✅ 弹窗控制系统测试成功！\n弹窗应该只显示一次。');
        } else {
            alert(`❌ 弹窗被阻止显示\n原因: ${checkResult.reason}\n显示次数: ${checkResult.display_count}/${checkResult.max_count}`);
        }
    } catch (error) {
        console.error('测试弹窗控制失败:', error);
...
async function resetModalState() {
    try {
        const confirmed = confirm('确定要重置所有弹窗状态吗？\n这将允许所有弹窗重新显示。');
        if (!confirmed) return;
